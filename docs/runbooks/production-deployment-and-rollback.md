...
This document details standard operating procedures for deploying, monitoring, and rolling back Mini-Vercel in production environments.
...
### Prerequisites
1. Dedicated Linux VM or bare-metal host with Docker 24+ and Docker Compose v2.
...
1. **Clone repository and configure environment**:
   ```bash
   git clone https://github.com/your-org/mini-vercel.git /opt/mini-vercel
   cd /opt/mini-vercel
...
Mini-Vercel exposes standard Prometheus metrics from Fastify API (`:8080/metrics`) and the BullMQ Worker process (`:9090/metrics` over internal container network).
...
```bash
docker compose -f deploy/docker-compose.production.yml -f deploy/docker-compose.observability.yml up -d prometheus grafana
```
...
Mini-Vercel uses immutable content-addressed storage in MinIO and atomic database pointer swaps.
...
If a new release of the Mini-Vercel platform itself introduces a regression:
...
3. Rebuild and restart the container services:
   ```bash
   docker compose -f deploy/docker-compose.production.yml up -d --build --force-recreate api worker web
   ```
4. Confirm health:
   ```bash
   docker compose -f deploy/docker-compose.production.yml ps
