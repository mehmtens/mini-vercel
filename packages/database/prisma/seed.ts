...
async function main() {
  ...
  const readyDeployment = await prisma.deployment.create({
    data: {
      ...
      previewUrl: 'https://nextjs-saas-starter-a1b2c3d.mini-vercel.app',
  ...
  await prisma.deploymentLog.createMany({
    data: [
      ...
      },
      {
        ...
        sequence: 4,
        stream: LogStream.STDOUT,
        logChunk: '[UPLOAD] Uploaded 28 static bundles to MinIO bucket mini-vercel-builds',
      },
      {
        deploymentId: readyDeployment.id,
        sequence: 5,
        stream: LogStream.STDOUT,
        logChunk: '[SUCCESS] Edge deployment complete -> https://nextjs-saas-starter-a1b2c3d.mini-vercel.app',
      },
    ],
  });
