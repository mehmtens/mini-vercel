...
async function main() {
  ...
  console.log(`[Doplo Cleanup] Starting resource garbage collection (DryRun: ${isDryRun})...`);
  ...
  console.log('[Doplo Cleanup] Results:');
  ...
  if (result.errors.length > 0) {
    console.error('[Doplo Cleanup] Errors encountered during cleanup:', result.errors);
    if (!isDryRun) {
      process.exit(1);
    }
  }

  console.log('[Doplo Cleanup] Finished successfully.');
  process.exit(0);
}

main().catch((err) => {
  console.error('[Doplo Cleanup] Fatal error during cleanup:', err);
  process.exit(1);
});
