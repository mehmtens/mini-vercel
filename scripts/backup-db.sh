#!/usr/bin/env bash
# ==============================================================================
# Doplo PostgreSQL Automated Backup & Retention Script
# Supports --dry-run for non-destructive pre-flight verification
# ==============================================================================
set -euo pipefail

BACKUP_DIR="${BACKUP_DIR:-/var/backups/doplo/postgres}"
RETENTION_DAYS="${RETENTION_DAYS:-7}"
TIMESTAMP="$(date +%Y%m%d_%H%M%S)"
BACKUP_FILE="${BACKUP_DIR}/doplo_${TIMESTAMP}.sql.gz"
...
if [ "$IS_DRY_RUN" = true ]; then
  ...
  echo "[$(date -Iseconds)] [DRY-RUN] Target database: ${POSTGRES_DB:-doplo} on ${POSTGRES_HOST:-localhost}:${POSTGRES_PORT:-5432}"
...
find "${BACKUP_DIR}" -name "doplo_*.sql.gz*" -type f -mtime +"${RETENTION_DAYS}" -delete
...
if mkdir -p "${METRICS_DIR}" 2>/dev/null; then
  echo "doplo_backup_last_success_timestamp_seconds{target=\"postgres\"} $(date +%s)" > "${METRICS_DIR}/doplo_backup_postgres.prom.$$" 2>/dev/null || true
  mv "${METRICS_DIR}/doplo_backup_postgres.prom.$$" "${METRICS_DIR}/doplo_backup_postgres.prom" 2>/dev/null || true
fi
